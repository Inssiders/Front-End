...
export const handlers = [
  // GET - 게시물 목록 조회 (와일드카드 패턴)
  http.get(`${BASE_URL}/api/posts`, ({ request }) => {
    console.log("[MSW] Intercepted GET request to (wildcard):", request.url);
    ...
    if (profileFilter) {
      ...
      console.log("[MSW] Returning", items.length, "items for profile filter");
    ...
    console.log("[MSW] Returning", items.length, "items for page", page);
  ...
  http.post(`${BASE_URL}/api/posts`, async ({ request }) => {
    console.log("[MSW] Intercepted POST request to (wildcard):", request.url);
  ...
  http.delete(`${BASE_URL}/api/posts/:id`, ({ params, request }) => {
    console.log("[MSW] Intercepted DELETE request to (wildcard):", request.url);
