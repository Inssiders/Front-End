...
export function MSWProvider({ children }: { children: React.ReactNode }) {
    ...
    const initMSW = async () => {
      ...
      try {
        ...
        if (!workerStarted) {
          ...
          console.error("[MSW Context] Worker started but interception test failed");
        } else {
        ...
        const errorMessage = `MSW initialization failed: ${(initError as Error).message}`;
  ...
  useEffect(() => {
    if (error) {
      ...
      if (process.env.NODE_ENV === "development") {
        setTimeout(() => {
          console.warn(`%c[MSW ERROR] ${error}`, "color: red; font-weight: bold; font-size: 14px;");
          console.warn(
  ...
  return <MSWContext.Provider value={{ isReady, error }}>{children}</MSWContext.Provider>;
}
