...
export default function DynamicVideoGrid({
  ...
  const [mutedVideos, setMutedVideos] = useState<Set<number>>(new Set(videoData.map((v) => v.id)))
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 auto-rows-[120px]">
        {videoData.map((video) => (
          <motion.div
            key={video.id}
            className={`${getSizeClasses(video.size)} relative rounded-lg overflow-hidden bg-mocha-700 ${
              !areFramesVisible ? "opacity-0 pointer-events-none" : "opacity-100"
            }`}
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2">
              <h3 className="text-cream-100 font-medium text-sm truncate">{video.title}</h3>
            </div>
