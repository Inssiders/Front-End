...
export default function ThreeScene({
  ...
  const [position, setPosition] = useState<[number, number, number]>([-1.5, -1, 0])
  ...
  useEffect(() => {
    ...
    if (scrollProgress < 0.33) {
      ...
      const leftPos = window.innerWidth / 2 - 75 + t * (window.innerWidth - 200 - (window.innerWidth / 2 - 75))
      const bottomPos = window.innerHeight / 2 - 75 + t * (50 - (window.innerHeight / 2 - 75))
      newLeft = `${leftPos}px`
      newBottom = `${bottomPos}px`
    }
